...
#1
puts "Il y a #{journalist.length} handles dans cette array"

#2
puts "Ce handle : #{journalist.min {|a, b| a.length <=> b.length}} est vraiment court"
...
puts "Il y a #{count.length} handles qui contiennent 5 caractères dans l'array"
...
puts "Il y a #{first_letter.length} qui commencent par une majuscule "

#5
puts journalist.sort  

#6
puts journalist.sort_by(&:length)

#7
puts "Ce journaliste ce trouve à la position : #{journalist.index("@epenser")}"
...
def length
  return journalist.length
end

def shortest
  return journalist.min_by(&:length)
end
...
def perform(journalist) 
  puts "Il y a #{journalist.length} handles dans cette array"
  puts "Ce handle : #{journalist.min_by(&:length)} est vraiment court"
  puts "Il y a #{count.length} handles qui contiennent 5 caractères dans l'array"
  puts "Ce journaliste ce trouve à la position : #{journalist.index("@epenser")}"
end
